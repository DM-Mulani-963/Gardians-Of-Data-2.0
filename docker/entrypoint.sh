#!/bin/bash
echo "Welcome to DATA Rakshak - Guardians of Data"

declare -r pyexe=python3
declare -r pymanage="$pyexe manage.py"
declare -r custom_docker_settings="/datarakshak/settings/custom_docker.py"
...
declare -Ar commandHelpList=(
	...
	[init]='Initialize database and launch DATA rakshak'
...
{
	cat <<- EOS
		$1: prepare and lauch DATA rakshak
...
function getSettings() {
	if [ -f "$custom_docker_settings" ]; then
		export DJANGO_SETTINGS_MODULE=DATA rakshak.settings.custom_docker
	else
		export DJANGO_SETTINGS_MODULE=DATA rakshak.settings.docker
		cat <<- EOS
...
function startWorker() {
	...
	exec celery -A DATA rakshak.core worker --beat -l info -S django
}
...
function init_db() {
	while ! pg_isready -h db -U datarakshak -p 5432 > /dev/null 2> /dev/null; do
		echo "Connecting to db (postgresql) Failed: Waiting ..."
		sleep 1
	done
	createDB
	importTrackers
	echo "DATA rakshak DB is ready."
}
